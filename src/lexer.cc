...
void Lexer::lex_file(const std::string& file_contents) {
    ...
    while (this->file_index < this->file_size) {
        ...
        if (whitespace.find(current_char) != whitespace.end()) {
            ...
            if (this->peek() == '=') {
                new_tok.set_type(token::LE);
                new_tok.set_value("<=");
                this->eat_char();
            } else {
                new_tok.set_type(token::LT);
                new_tok.set_value("<");
            }
            ...
            if (this->peek() == '=') {
                new_tok.set_type(token::GE);
                new_tok.set_value(">=");
                this->eat_char();
            } else {
                new_tok.set_type(token::GT);
                new_tok.set_value(">");
            }
            ...
            this->token_list.push_back(this->eat_ident());
...
token::Token Lexer::eat_ident() {
    token::Token new_tok;
    int file_start = this->file_index;
    new_tok.set_row(this->row);
    new_tok.set_col(this->col);

    while (this->file_index < this->file_size && (isalnum(this->cur_char()) || this->cur_char() == '_')) {
        this->eat_char();
    }
    std::string cur_substr = this->file_contents.substr(file_start, this->file_index - file_start);

    if (reserved_idents.find(cur_substr) != reserved_idents.end()) {
        new_tok.set_type(reserved_idents.at(cur_substr));

    } else {
        new_tok.set_type(token::IDENTIFIER);

    }

    new_tok.set_value(cur_substr);
